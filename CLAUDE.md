...
A dotted command name (e.g. `tb1.kino.bahamut`) **must start with its own module's name** — `add_route` raises `ValueError` at registration time otherwise, so one module's commands can never show up filed under another's in `/help`'s drill-down tree (`command_help.build_help_text`, grouped by splitting each command's dotted name on `.`, not by which module advertised it).
